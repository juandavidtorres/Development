    ...
    public static class Util
    {
        ...
        public static ServiceControllerStatus ServiceStatus(string serviceName)
        {
            try
            {
                ...
                else throw new Exception("Servicio no encontrado.");
            }
            catch { throw; }
        }
        ...
        {
            FileInfo info = new FileInfo(Util.LibraryFullPath());
        ...
        public static string GetFilterExtensions(string strExtensions)
        {
        ...
        public static void EnabledControls(Control container, bool enabled)
        {
            foreach (Control control in container.Controls)
                if (control is UserControl || control is ScrollableControl || control is Panel)
                    Util.EnabledControls(control, enabled);
        ...
        {
            if (character == Util.KeyTab ||
                character == Util.KeyReturn ||
                character == Util.KeyNewLine)
                return true;
            else return false;
        }
        ...
        public static Image GetImageFromString(string image)
        {
            ...
            {
                MemoryStream ms = new MemoryStream(Util.ToBytes(image));
        ...
        //Devuelve la codificacion de caracteres de un archivo de texto
        ...
        public static string ExportTableToFile(DataTable table, char separator = '\t')
        {
            try
            {
