...
Public Class ServiceTerminalHost
    ...
    Dim dicFacturas As New Dictionary(Of String, Factura)
...
#Region "Definicion de Eventos"
    ...
    Public Event oEventos_ReimprimirTurnoFrecuenciaPorEmpleado(IdEmpleado As String, Fecha As String, Puerto As String)
    ...
    Public Event OEventos_EnviarRemarcaciones(ByRef codTanqueBaja As String, ByRef productoBaja As Int16, ByRef cantidadRemarcar As Double, ByRef codTanqueAlta As String, ByRef productoAlta As Int16, ByRef puerto As String)
    ...
    Public Event oEventos_VentaCreditoCanastilla(ByVal MCIdentificacion As MedioCredito, ByVal MCCanastilla As ColCanastilla, ByVal IdEmpleado As String, ByVal Password As String, ByVal Isla As Integer, ByVal puerto As String)
    ...
    Public Event oEventos_EnviarAjusteDeTanquesVeederRoot(ByVal Puerto As String)

#End Region


    Public Function RecuperarParametro(Nombre As String) As String Implements IServiceTerminalHost.RecuperarParametro
        Dim oHelper As New Helper

        Try
            Return oHelper.RecuperarParametro(Nombre)
        Catch Ex As System.Exception
            LogCategorias.Clear()
            LogCategorias.Agregar("Anomalia")
            LogPropiedades.Clear()
            LogPropiedades.Agregar("Mensaje", Ex.Message)
            Fabrica.LogIt.Loguear("ERROR en RecuperarParametro: ", LogPropiedades, LogCategorias)
            Throw New System.Exception("ERROR en RecuperarParametro: " & vbNewLine & Ex.Message)
        End Try
    End Function
    ...
    Public Sub AplicarModificacionVenta(Recibo As String, ByVal ListaMedios As List(Of DTOMedioPago), ImprimirTicketModificacion As Boolean, puerto As String) Implements IServiceTerminalHost.AplicarModificacionVenta
        ...
        Try
            ...
            For Each oPago As DTOMedioPago In ListaMedios
                ...
                Pago.NroTarjeta = ""
                Pago.Valor = CDbl(Utilidades.ModificarFormatoDecimal(oPago.Valor))
                Pago.TipoLectura = "0"
    ...
    Function VerificarBonoSodexo(ByVal NroConfirmacion As String, ByVal IdFormaPago As String) As RespuestaVerificarBonoSodexo Implements IServiceTerminalHost.VerificarBonoSodexo
        Dim oRespuesta As New RespuestaVerificarBonoSodexo
        Dim oHelper As New Helper
        Dim oRes As Boolean
        Try
            oRes = oHelper.VerificarBonoSodexo(NroConfirmacion, CInt(IdFormaPago))
    ...
    Function RecuperarProductosCanastilla(ByVal Codigo As String, isla As Integer) As DTOProducto Implements IServiceTerminalHost.RecuperarProductoCanastilla
        Dim oValor As New DTOProducto
        Try
            Dim oHelper As New POSstation.HelperCanastilla.Helper
            Dim ohelperX As New Helper
            Dim oProducto As IDataReader = Nothing
            If Not CBool(ohelperX.RecuperarParametro("AplicaInventarioPorIsla")) Then isla = -1

            oProducto = oHelper.RecuperarProductoPorCodigo(CInt(Codigo), isla)
    ...
    Public Function GuardarFacturaCanastilla(ByVal NumeroTarjeta As String, ByVal TipoLectura As Byte, ByVal NroSeguridad As String, ListaProductos As List(Of DTOVentaCanastilla), Puerto As String) As RespuestaDescuento Implements IServiceTerminalHost.GuardarFacturaCanastilla
        ...
        Dim oFactura As Factura = Nothing
        ...
        Try
            ...
            For Each oProducto As DTOVentaCanastilla In ListaProductos
                oFactura.AgregarProductoTerpel(oProducto.Codigo, oProducto.Cantidad, Puerto)
            Next
            ...
            If (dicFacturas.ContainsKey(Puerto)) Then dicFacturas.Remove(Puerto)
            dicFacturas.Add(Puerto, oFactura)
    ...
    Public Sub GuardarFacturaCanastillaCredito(ByVal Cara As Byte, ByVal NumeroTarjeta As String, ByVal TipoLectura As Byte, ByVal NumeroAutorizacionManual As String, ByVal tipoIdentificacion As String, ListaProductos As List(Of DTOVentaCanastilla), IdEmpleado As String, Password As String, Puerto As String, ByVal MediosLectura As MediosLectura) Implements IServiceTerminalHost.GuardarFacturaCanastillaCredito
        Try
            ...
            If (dicFacturas.ContainsKey(Puerto)) Then oFactura = dicFacturas(Puerto)

            If Not oFactura Is Nothing Then
                Dim IdVentaCanastilla As Int32 = 0
                Dim TotalFactura As Double = oFactura.RecuperarTotalventa()
                Dim isla As Integer = IIf(CBool(oHelper.RecuperarParametro("AplicaInventarioPorIsla")), Puerto, -1)
                    ...
                    If oFactura.NoAplicaDescuentoCDC Then
                        ...
                        If oFactura.Productos.Count > 0 Then
                            For i As Int16 = 1 To oFactura.Productos.Count
                                ...
                                MCCanastilla.Add(codigo, cantidad, isla, "")
                            Next

                            'Disparamos el evento de Canastilla    
                            RaiseEvent oEventos_VentaCreditoCanastilla(MCIdentificacion, MCCanastilla, IdEmpleado, Password, isla, Puerto)
        ...
        Try
            ...
            If Not oFactura Is Nothing Then
                ...
                If Not IsNothing(oFactura) Then
                    ...
                    If oFactura.NoAplicaDescuentoCDC Then
                        MCIdentificacion.Cara = Puerto '0 'MediosLectura.Cara
                        Dim producto As FacturaDetalle = New FacturaDetalle()
                        Dim codigo As String = ""
                        Dim cantidad As Double = 0
                        ...
                        If oFactura.Productos.Count > 0 Then
                            ...
                            'Disparamos el evento de Canastilla                            
                            IdVentaCanastilla = VentaCanastillaEfectivo(MCIdentificacion, MCCanastilla, IdEmpleado, Password, Puerto)
    ...
    Private Function VentaCanastillaEfectivo(ByVal identificacion As MedioCredito, ByVal colProductos As ColCanastilla, ByVal IdEmpleado As String, ByVal Password As String, ByVal Puerto As String) As Integer
        Try
            Dim oDataAccess As New Helper
    ...
    Sub ImprimirCopiaTurnoFrecuencia(IdEmpleado As String, Fecha As String, Puerto As String) Implements IServiceTerminalHost.ImprimirCopiaTurnoFrecuencia
        Try
            RaiseEvent oEventos_ReimprimirTurnoFrecuenciaPorEmpleado(IdEmpleado, Fecha, Puerto)
        Catch ex As System.Exception
            LogCategorias.Clear()
            LogCategorias.Agregar("Anomalia")
            LogPropiedades.Clear()
            LogPropiedades.Agregar("Mensaje", ex.Message)
            Fabrica.LogIt.Loguear("Excepcion en Imprimir Copia de Turno Frecuencia por Codigo de Empleado", LogPropiedades, LogCategorias)
            Throw
        End Try
    End Sub
    ...
    Public Function SaldoTarjetaPrepago(NoTarjeta As String, Puerto As String) As String Implements IServiceTerminalHost.SaldoTarjetaPrepago
        ...
        Try
            ...
            saldo = ServerServices.CDCServices.SaldoTarjeta(NoTarjeta)
            Datos = CInt(saldo.Saldo).ToString() + "|" + NoTarjeta
            saldoTarjeta = saldo.Saldo.ToString
    ...
    Public Function DetalleMovimientos(NoTarjeta As String, Puerto As String) As DetalleMovimientos Implements IServiceTerminalHost.DetalleMovimientos
        ...
        Try
            ...
            DMov.Cliente = saldo.Cliente
            DMov.Saldo = CDec(saldo.Saldo)
    ...
    Public Function MovimientosPrepago(NoTarjeta As String, Puerto As String) As List(Of Movimiento) Implements IServiceTerminalHost.MovimientosPrepago
        ...
        Try
            Dim listas As List(Of ServerServices.MovimientoTarjeta) = ServerServices.CDCServices.MovimientosPrepago(NoTarjeta)
            DMovimientos = New List(Of Movimiento)
            For Each item As ServerServices.MovimientoTarjeta In listas
                Mov = New Movimiento
                Mov.FechaHora = item.FechaHora
    ...
    Private Sub PredeterminarTarjeta(cara As Byte, noTarjeta As String, valor As String, puerto As String) Implements IServiceTerminalHost.PredeterminarTarjeta
        ...
        Try
            Dim saldo As New RespuestaSaldo
            saldo = ServerServices.CDCServices.SaldoTarjeta(noTarjeta)
            If saldo.Saldo < CDbl(valor) Then
                Throw New System.Exception("El valor digitado es mayor que el actual saldo de la tarjeta")
            End If
    ...
    Public Sub PredeterminarCheque(Cara As Byte, NumAutorizacion As String, Placa As String, NumeroTarjeta As String, TipoLectura As Byte, NroSeguridad As String,
    ...
    Public Sub ValidarTarjetaRecarga(ByVal NumeroTarjeta As String, ByVal Valor As String) Implements IServiceTerminalHost.ValidarTarjetaRecarga
        ...
        Try
            ...
            Dim saldoconsulta As New RespuestaSaldo
            saldoconsulta = ServerServices.CDCServices.SaldoTarjeta(NumeroTarjeta)
            Saldo = saldoconsulta.Saldo
            If saldoconsulta.Saldo < CDbl(Valor) Then
...
#Region "Fidelizacion"
    ...
    Public Sub FidelizarVenta(Cara As Byte, NumeroTarjeta As String, IdRedFidelizacion As Short, Tipo As String, TipoVenta As String, Puerto As String) Implements IServiceTerminalHost.FidelizarVenta
        ...
        Try
            ...
            NumeroTarjeta = odatos.ValidarNumeroTarjeta(NumeroTarjeta)
    ...
    Public Sub PagoConBonoSauce(Ticket As String, NumeroTarjeta As String, TipoLectura As Byte, Puerto As String) Implements IServiceTerminalHost.PagoConBonoSauce
        ...
        Try
            Dim odatos As New Helper
            Dim recibo As String = odatos.RecuperarReciboPorCara(Ticket)
...
#Region "Redencion Bono-Petromil"
    ...
    Public Function ConsultarSaldoTarjeta(ByVal NumTarjeta As String, ByVal TipoLectura As Int16, ByVal esImpresion As Boolean, ByVal impresora As String) As String
        ...
        Try
            Dim odatos As New Helper
            NumTarjeta = odatos.ValidarNumeroTarjeta(NumTarjeta)
...
#End Region
    ...
    Public Sub AnularFacturaCanastilla(ByVal Prefijo As String, ByVal Consecutivo As String, ByVal IdEmpleado As String, ByVal Password As String, ByVal Puerto As String) Implements IServiceTerminalHost.AnularFacturaCanastilla
        ...
        Try
            ...
            If oDataAccess.ValidarFacturaTerpel(Prefijo, CInt(Consecutivo)) Then
                'StringTx = "Y|";
    ...
    Public Sub ImprimirCopiaRecargaPrepago(ByVal Consecutivo As String, ByVal Puerto As String) Implements IServiceTerminalHost.ImprimirCopiaRecargaPrepago
        Dim oHelper As New Helper
        Try
            RaiseEvent oEventos_reImprimirRecargaPrepago(CInt(Consecutivo), Puerto)
        ...
        Catch Ex As System.Exception
            ...
            LogPropiedades.Clear()
            LogPropiedades.Agregar("Mensaje", Ex.Message)
            Fabrica.LogIt.Loguear("ERROR en [ImprimirCopiaRecargaPrepago]: ", LogPropiedades, LogCategorias)
            Throw New System.Exception("ERROR en [ImprimirCopiaRecargaPrepago]: " & vbNewLine & Ex.Message)
        End Try
    End Sub

    Public Sub CierreTanquesVeederRoot(ByVal Puerto As String) Implements IServiceTerminalHost.CierreTanquesVeederRoot
        Try
            RaiseEvent oEventos_EnviarAjusteDeTanquesVeederRoot(Puerto)
        Catch ex As System.Exception
            Throw ex
        End Try
    End Sub
    Public Function CopiaPagoFrecuencia(IdRecaudo As String, IdIsla As String) As RespuestaGeneral Implements IServiceTerminalHost.CopiaPagoFrecuencia
        Dim respuesta As New RespuestaGeneral
        Dim Nombreimpresora As String = ""
        Dim odatos As New Helper
        Dim impresora As New ImpresoraEstacion
        Dim ORecaudo As IDataReader = Nothing

        Try
            ORecaudo = odatos.RecuperarRecaudoEmpresa(IdRecaudo)
            If ORecaudo.Read Then
            End If
            ORecaudo.Close()
            ORecaudo.Dispose()

            Nombreimpresora = odatos.RecuperarImpresoraPorIsla(IdIsla)
            impresora.Nombre = Nombreimpresora
            impresora.ESCopia = True
            ImpresoraRecibo.ImprimirPagoFrecuencia(IdRecaudo, impresora)
            respuesta.EsAutorizado = True
            respuesta.Mensaje = ""
        Catch ex As System.Exception
            LogCategorias.Clear()
            LogCategorias.Agregar("Anomalia")
            LogPropiedades.Clear()
            LogPropiedades.Agregar("Mensaje", ex.Message)
            Fabrica.LogIt.Loguear("ERROR en [CopiaPagoFrecuencia]", LogPropiedades, LogCategorias)
            respuesta.EsAutorizado = False
            respuesta.Mensaje = ex.Message
            Try
                If Not ORecaudo.IsClosed Then
                    ORecaudo.Close()
                End If
            Catch
        ...
        Return respuesta
    End Function
    Public Function RealizarPagoInfoTaxi(Usuario As String, Clave As String, ByVal Placa As String, ByVal Movil As Int32, ByVal IdConcepto As Int32, ByVal CantidadPeriodos As Int32, ByVal Valor As Decimal, Recibo As String, Isla As String) As RespuestaGeneral Implements IServiceTerminalHost.RealizarPagoInfoTaxi
        Dim respuesta As New RespuestaGeneral
        Dim ODataSet As New DataSet
        Dim CodEstacion As String = ""
        Dim Data As RespuestaPagoInfoTaxi = Nothing
        Dim IdTurno As Integer = 0
        Dim IdRecaudo As String = ""
        Dim odatos As New Helper
        Dim Nombreimpresora As String = ""
        Dim impresora As New ImpresoraEstacion
        Try
            Nombreimpresora = odatos.RecuperarImpresoraPorIsla(Isla)
            ODataSet = odatos.RecuperarCodEstacion()
            CodEstacion = ODataSet.Tables(0).Rows(0).Item("Codigo").ToString()
            ODataSet = Nothing
            IdTurno = odatos.ValidarTurnoPorEmpleado(Usuario, Clave)
            If IdTurno = -1 Then
                Throw New System.Exception("El empleado " & Usuario & " no tiene un turno abierto")
            ElseIf IdTurno = 0 Then
                Throw New System.Exception("El empleado " & Usuario & " no existe en la base de datos")
            End If
            Try
                IdRecaudo = odatos.InsertarRecaudoInfoEmpresa(Movil, CantidadPeriodos, IdConcepto, IdTurno, Recibo, Placa, Valor, Nothing, Nothing)
                Data = ServerServices.CDCServices.RealizarPagoInfoTaxi(Placa, Movil, IdRecaudo, IdConcepto, CantidadPeriodos, Valor, CodEstacion, odatos.RecuperarParametro("UrlInfoTaxi"))

                If Data.EsProcesado Then
                    odatos.ActualizarPagoInfoTaxi(IdRecaudo, Data.IdPago, Data.Respuesta, Data.Mensaje, False, True, Data.Fecha)
                    impresora.Nombre = Nombreimpresora
                    ...
                    respuesta.Mensaje = ""
                Else
                    odatos.ActualizarPagoInfoTaxi(IdRecaudo, Nothing, Data.Respuesta, Data.Mensaje, True, False, Nothing)
                    Throw New System.Exception("ERROR COMUNICACION: " & Data.Mensaje)
                End If
            ...
            Catch ex As System.Exception
                ...
                LogPropiedades.Clear()
                LogPropiedades.Agregar("Mensaje", ex.Message)
                Fabrica.LogIt.Loguear("ERROR en [RealizarPagoInfoTaxi] COMUNICACION", LogPropiedades, LogCategorias)
                respuesta.EsAutorizado = False
                respuesta.Mensaje = ex.Message
                Dim res As String = ""
                Dim mensaje As String = ""
                ...
                End If
                odatos.ActualizarPagoInfoTaxi(IdRecaudo, Nothing, res, mensaje, True, False, Nothing)
            End Try
        Catch ex As System.Exception
            LogCategorias.Clear()
            LogCategorias.Agregar("Anomalia")
            LogPropiedades.Clear()
            LogPropiedades.Agregar("Mensaje", ex.Message)
            Fabrica.LogIt.Loguear("ERROR en [RealizarPagoInfoTaxi] GENERAL", LogPropiedades, LogCategorias)
            respuesta.EsAutorizado = False
            respuesta.Mensaje = ex.Message
            Dim res As String = ""
            Dim mensaje As String = ""
        ...
        Return respuesta
    End Function
    Function AbrirTurnoRecaudo(Usuario As String, Clave As String) As RespuestaGeneral Implements IServiceTerminalHost.AbrirTurnoRecaudo
        Dim respuesta As New RespuestaGeneral
        Try
            Dim odato As New Helper
            odato.AbrirTurnoRecaudo(Usuario, Clave)
            respuesta.EsAutorizado = True
            respuesta.Mensaje = ""
        Catch ex As System.Exception
            LogCategorias.Clear()
            ...
            LogPropiedades.Clear()
            LogPropiedades.Agregar("Mensaje", ex.Message)
            Fabrica.LogIt.Loguear("ERROR en [AbrirTurnoRecaudo]", LogPropiedades, LogCategorias)
            respuesta.EsAutorizado = False
            respuesta.Mensaje = ex.Message
        End Try
        Return respuesta
    ...
    Function CerrarTurnoFrecuencias(Usuario As String, Clave As String, Isla As String) As RespuestaGeneral Implements IServiceTerminalHost.CerrarTurnoFrecuencias
        Dim respuesta As New RespuestaGeneral
        Dim IdTurno As Integer = 0
        Dim Nombreimpresora As String = ""
        Dim impresora As New ImpresoraEstacion
        ...
        Try
            ...
            impresora.Nombre = Nombreimpresora
            ImpresoraTurno.ImprimirTurnoPagoFrecuencia(IdTurno, impresora)
    ...
    Function AuditoriaTurnoFrecuencias(Usuario As String, Clave As String, Isla As String) As RespuestaGeneral Implements IServiceTerminalHost.AuditoriaTurnoFrecuencias
        Dim respuesta As New RespuestaGeneral
        Dim IdTurno As Integer = 0
        Dim Nombreimpresora As String = ""
        Dim impresora As New ImpresoraEstacion
        ...
        Try
            ...
            LogCategorias.Clear()
            LogCategorias.Agregar("SeguimientoRecaudo")
            LogPropiedades.Clear()
            LogPropiedades.Agregar("Impresora", Nombreimpresora)
            LogPropiedades.Agregar("Isla", Isla)
            LogPropiedades.Agregar("Usuario", Usuario)
            LogPropiedades.Agregar("Clave", Clave)
            LogPropiedades.Agregar("IdTurno", IdTurno)
            Fabrica.LogIt.Loguear("Seguimiento en [AuditoriaTurnoFrecuencias]", LogPropiedades, LogCategorias)
            impresora.Nombre = Nombreimpresora
            ImpresoraTurno.ImprimirTurnoPagoFrecuencia(IdTurno, impresora, True)
            respuesta.EsAutorizado = True
            respuesta.Mensaje = ""
        Catch ex As System.Exception
            LogCategorias.Clear()
            ...
            LogPropiedades.Clear()
            LogPropiedades.Agregar("Mensaje", ex.Message)
            Fabrica.LogIt.Loguear("ERROR en [AuditoriaTurnoFrecuencias]", LogPropiedades, LogCategorias)
            respuesta.EsAutorizado = False
            respuesta.Mensaje = ex.Message
        End Try
        Return respuesta
