...
Public Class ImpresoraTurno
    ...
    Public Shared Sub ImprimirMovimientosPrepago(ByVal Tarjeta As String, ByVal impresora As ImpresoraEstacion)
        Try

            Dim OHelper As New Helper
            Dim OMovimientos As IDataReader = OHelper.RecuperarMovimientoTarjetaPrepago(Tarjeta)
            ' Dim OMovimientosCanastilla As IDataReader = OHelper.RecuperarMovimientoTarjetaPrepagoCanastilla(Tarjeta)
            Dim ODatosTarjeta As IDataReader = OHelper.RecuperarDatosTarjetaPrepago(Tarjeta)
            ...
            Using SW As StreamWriter = New StreamWriter(My.Application.Info.DirectoryPath & "\TempTurnos\ImprimirMovimientosPrepago" & Tarjeta.ToString() & ".txt")
                ...
                If ODatosTarjeta.Read Then
                    ...
                    SW.WriteLine("NOMBRE: " & ODatosTarjeta.Item("NombreCliente").ToString)
                    SW.WriteLine("TARJETA: " & Tarjeta)
                    SW.WriteLine("SALDO: " & CDbl(ODatosTarjeta.Item("Saldo").ToString).ToString("N2"))
                End If
                SW.WriteLine("---------------------------------------")
                ...
                If OMovimientos.Read Then
                    ...
                    SW.WriteLine(DobleColumna(20, OMovimientos.Item("Valor").ToString, OMovimientos.Item("FechaHora").ToString))
        ...
        Try
            ...
            If Idturno <> "0" Then
                ...
                If Not My.Computer.FileSystem.DirectoryExists(My.Application.Info.DirectoryPath & "\TempTurnos") Then
                    My.Computer.FileSystem.CreateDirectory(My.Application.Info.DirectoryPath & "\TempTurnos")
                End If
                If File.Exists(My.Application.Info.DirectoryPath & "\TempTurnos\Turno" & Idturno & ".txt") Then
                    File.Delete(My.Application.Info.DirectoryPath & "\TempTurnos\Turno" & Idturno & ".txt")
                End If

                '--------------------------INICIA LA CONSTRUCCION DEL REPORTE----------------------------------------------------------
                Using SW As StreamWriter = New StreamWriter(My.Application.Info.DirectoryPath & "\TempTurnos\Turno" & Idturno & ".txt")
                    ...
                    lector = OHelper.RecuperarVentasTotalizadasPorTurno(Idturno)
                    Cantidad = 0
                    Valor = 0
                    SW.WriteLine("========================================")
                    SW.WriteLine(CentrarTexto(40, "RECARGAS PREPAGO"))
                    ...
                    While OResultadoDatos.Read
                        ...
                        SW.WriteLine(DobleColumnaTotal("Gran Total", (CDbl(OResultadoDatos.Item("Ingresos")) - CDbl(OResultadoDatos.Item("Reversos")) + CDbl(OResultadoDatos.Item("Recaudos")) + CDbl(OResultadoDatos.Item("Combustible")) - CDbl(OResultadoDatos.Item("Descuentos")) + CDbl(OResultadoDatos.Item("Impuestos")) + CDbl(OResultadoDatos.Item("Incrementos")) + CDbl(OResultadoDatos.Item("Canastilla")) + CDbl(OResultadoDatos.Item("TotalPrepago")).ToString("N2"))))
                    End While
                    ...
                    '----------------------IMPRIMO EL ARCHIVO Y POSTERIORMENTE LO ELIMINO-----------------------------------
                    AsciiToPrinter(My.Application.Info.DirectoryPath & "\TempTurnos\Turno" & Idturno & ".txt", impresora.Nombre)
                    File.Delete(My.Application.Info.DirectoryPath & "\TempTurnos\Turno" & Idturno & ".txt")
                End Using
            End If
        Catch ex As Data.DataException
