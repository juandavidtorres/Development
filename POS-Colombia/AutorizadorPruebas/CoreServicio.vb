...
#Region "Multihilo"
                    ...
                    If Credito.EsEfectivo Then
                        If Credito.EsClienteIdentificado Then
                            If Credito.TipoIdentificacion = TipoIdentificador.CHIP Then
                                ...
                                oEventos_VentaAutorizada(cara, preciosurtidor, OPreset.Valor, CByte(OPreset.TipoPreset), placatemporal, idManguera, False)
                            End If
                        End If
                    End If

                ElseIf CreditosTerpel.ContainsKey(cara.ToString()) Then
                    AutorizarVentaCDCTerpel(cara, preciosurtidor, OPlaca.Rom, OPreset)
                Else
                    If esLecturaChipObligatoria And OHelper.RecuperarParametro("AutorizarVentaLiquidoConChip") = False Then
                        ...
                        oEventos_VentaAutorizada(cara, preciosurtidor, OPreset.Valor, CByte(OPreset.TipoPreset), OPlaca.Placa, idManguera, False)
                    Else
                        If CBool(OHelper.RecuperarParametro("IdentificarVentasEfectivoConChip")) Then
                            ...
                            oEventos_VentaAutorizada(cara, preciosurtidor, OPreset.Valor, CByte(OPreset.TipoPreset), Placa, idManguera, False)
    ...
    Private Function AutorizarVentaMenoCash(ByVal Cara As String, ByVal idProducto As Int32, ByVal Kilometraje As Int64, ByVal preciosurtidor As Double, ByVal idManguera As Int32, ByVal Rom As String, ByVal OPreset As Preset)
        Dim oHelper As New Helper
        Dim Gst1 As New GSTCriptografia1.Criptografia1
        Dim oDatosMenoCash As New MenoCash
        Dim oReaderMeno As IDataReader
        Dim EsAutorizado As Boolean
        Try
            oDatosMenoCash.CodEstacion = oHelper.RecuperarParametro("CodigoEstacionGasoNet").ToString()
            oDatosMenoCash.DireccionIp = oHelper.RecuperarParametro("CodigoSeguridadGasoNet").ToString()
            oDatosMenoCash.Rom = Rom
            oDatosMenoCash.NumAuditoria = oHelper.RecuperarConsecutivo("NumAuditoria").ToString()
            oReaderMeno = oHelper.RecuperarDatosMenoCash(Int32.Parse(Cara), idProducto)
            If oReaderMeno.Read() Then
                oDatosMenoCash.Producto = Int32.Parse(oReaderMeno("CodigoMenoCash").ToString())
                oDatosMenoCash.UnidadMedidad = Int32.Parse(oReaderMeno("CodMenoCashUnidad").ToString())
                oDatosMenoCash.Isla = Int32.Parse(oReaderMeno("IdIsla").ToString())
                oDatosMenoCash.Surtidor = Int32.Parse(oReaderMeno("IdSurtidor").ToString())
            End If
            oDatosMenoCash.Manguera = idManguera
            Dim LlavePublica = oHelper.RecuperarParametro("LlavePublica").ToString()
            Dim LlavePrivada = oHelper.RecuperarParametro("LlavePrivada").ToString()
            Dim CodEstacion = Gst1.EncryptString(oDatosMenoCash.CodEstacion, 1024, LlavePublica)
            Dim DireccionIp = Gst1.EncryptString(oDatosMenoCash.DireccionIp, 1024, LlavePublica)
            Dim oRom = Gst1.EncryptString(Rom, 1024, LlavePublica)
            Dim NumAuditoria = Gst1.EncryptString(oDatosMenoCash.NumAuditoria, 1024, LlavePublica)
            Dim servMeno As New ServicioMeno_Cash.IntegracionServiceCore
            Dim hab As New POSstation.ServerServices.ServicioMeno_Cash.preautorizacionDTO
            Dim Respuesta As POSstation.ServerServices.ServicioMeno_Cash.respuestaPreDTO
            servMeno.Url = oHelper.RecuperarParametro("UrlMenoCash")
            hab.codigoEstacion = CodEstacion
            hab.direccionIP = DireccionIp
            hab.ROM = oRom
            hab.numAuditoria = NumAuditoria
            hab.producto = oDatosMenoCash.Producto
            hab.unidadMedida = oDatosMenoCash.UnidadMedidad
            hab.precio = preciosurtidor
            hab.isla = oDatosMenoCash.Isla
            hab.surtidor = oDatosMenoCash.Surtidor
            hab.cara = Cara
            hab.manguera = idManguera
            hab.kilometros = 0
            Respuesta = servMeno.Preautorizar(hab)
            If Respuesta.codigo = "00" Then
                If Respuesta.valor > 0 Then
                    OPreset.Valor = Respuesta.valor
                    OPreset.TipoPreset = Preset.Tipo.PorValor
                Else
                    OPreset.Valor = Respuesta.valor
                    OPreset.TipoPreset = Preset.Tipo.PorVolumen
                End If
                Dim Numero = Gst1.DecryptString(Respuesta.numero, 1024, LlavePrivada)
                oHelper.InsertarActualizarVentaGasoNet(CInt(Cara), Respuesta.codigo, Nothing, Respuesta.mensaje, Nothing, CStr(Respuesta.fecha), Nothing, Numero, Nothing, Nothing, CDec(Respuesta.valor), Nothing, 1, Nothing, Nothing, Nothing)
                oEventos_VentaAutorizada(Cara, preciosurtidor.ToString(), CDec(OPreset.Valor).ToString("N2"), OPreset.TipoPreset, "", -1, True)
                EsAutorizado = True
            Else
                ImpresoraTurno.ImprimirExcepcion(Respuesta.mensaje, ImpresoraEstacion.CrearImpresora(Cara))
                EsAutorizado = False
        ...
        Catch ex As System.Exception
            EsAutorizado = False
            oHelper.VerificarVentaGasoNet(CInt(Cara.ToString()))
            DatosMenoCash.Remove(Cara.ToString())
            LogFallas.ReportarError(ex, "AutorizarVentaMenoCash", "Cara: " & Cara & ", PrecioPreducto: " & preciosurtidor, "AutorizarVentaMenoCash")
            Throw
        End Try

    End Function
    Private Function AutorizarVentaCDCTerpel(ByVal Cara As Int16, ByVal PrecioPreducto As Decimal, ByVal Identificacion As String, ByVal OPreset As Preset) As Boolean
        Dim OHelper As New Helper
        Dim EsAutorizado As Boolean
            ...
            If Not RespuestaCDCTerpel Is Nothing Then
                If RespuestaCDCTerpel.Procesado = True Then
                    ...
                    oEventos_VentaAutorizada(Cara, OHelper.RecuperarPrecioPorCliente(OCredito.Placa).ToString("N"),
                                             CDec(OPreset.Valor).ToString("N2"), OPreset.TipoPreset, OCredito.Placa, -1, True)
                    EsAutorizado = True
                Else
                    RomEstacion(Cara.ToString) = ""
                    If CreditosTerpel.ContainsKey(Cara.ToString) Then
    ...
    Public Sub AutorizarVentaCredito(ByVal esLecturaChipObligatoria As Boolean, ByVal cara As Byte, ByVal idProducto As Int32, ByVal idManguera As Int16, ByVal Identificador As String, ByVal TipoIdentificador As TipoIdentificadorVehiculo, ByRef OPreset As Preset, Optional esAutorizado As Boolean = True)
        Dim OHelper As New Helper
        ...
        If OPreset.TipoPreset = Preset.Tipo.PorValor And OPreset.Valor > 0 Then
            If OPreset.Valor > CreditosEstacion(cara.ToString()).ValorPredeterminado And CreditosEstacion(cara.ToString()).ValorPredeterminado > 0 Then
                OPreset.Valor = CreditosEstacion(cara.ToString()).ValorPredeterminado
            End If
        ElseIf OPreset.TipoPreset = Preset.Tipo.PorVolumen And OPreset.Valor > 0 Then
            If OPreset.Valor * PrecioProducto > CreditosEstacion(cara.ToString()).ValorPredeterminado And CreditosEstacion(cara.ToString()).ValorPredeterminado > 0 Then
                OPreset.Valor = CreditosEstacion(cara.ToString()).ValorPredeterminado
                OPreset.TipoPreset = Preset.Tipo.PorValor
            Else
                OPreset.Valor = OPreset.Valor * PrecioProducto
                OPreset.TipoPreset = Preset.Tipo.PorValor
            End If
        Else
            OPreset.Valor = CreditosEstacion(cara.ToString()).ValorPredeterminado
        ...
        If (OHelper.ExisteManejoCarroTanque(cara) And esAutorizado) Or Not OHelper.ExisteManejoCarroTanque(cara) Then
            ...
            oEventos_VentaAutorizada(cara, preciosurtidor, OPreset.Valor, CByte(OPreset.TipoPreset), Placa, idManguera, True)
            OPreset.Valor = 0
        End If
    End Sub
...
#Region "Eventos de Apertura y Cierre de Turnos"
    ...
    Private Sub oEventos_CierreTurno(empleado As String, clave As String, puerto As String, SurtidoresACerrar As String)
        ...
        Try
            ...
            End If
        Catch EXBD As Data.DataException
            OHelper.InsertarTurnoFallidoLogueo(empleado, clave, "", "Exception en Cierre de Turno -" + EXBD.Message.ToString(), puerto, "ImpresionTurnos")
            LogFallas.ReportarError(EXBD, "oEventos_CierreTurno", "Empleado: " & empleado & ", Clave: " & clave & ", Puerto: " & puerto, "ImpresionTurnos")
            oEventos_CierreTurnoFallido(EXBD.Message, puerto)
        Catch EXSQL As Data.SqlClient.SqlException
            OHelper.InsertarTurnoFallidoLogueo(empleado, clave, "", "Exception en Cierre de Turno -" + EXSQL.Message.ToString(), puerto, "ImpresionTurnos")
            LogFallas.ReportarError(EXSQL, "oEventos_CierreTurno", "Empleado: " & empleado & ", Clave: " & clave & ", Puerto: " & puerto, "ImpresionTurnos")
            oEventos_CierreTurnoFallido(EXSQL.Message, puerto)
        Catch ex As System.Exception
            OHelper.InsertarTurnoFallidoLogueo(empleado, clave, "", "Exception en Cierre de Turno -" + ex.Message.ToString(), puerto, "ImpresionTurnos")
            LogFallas.ReportarError(ex, "oEventos_CierreTurno", "Empleado: " & empleado & ", Clave: " & clave & ", Puerto: " & puerto, "ImpresionTurnos")
...
#Region "Eventos y Funciones de Venta "
    ...
    Private Sub oEventos_AutorizacionRequerida(cara As Byte, producto As Integer, idManguera As Integer, lectura As String)
        ...
        Try
            ...
            LogPropiedades.Agregar("Mensaje", "Antes de la Hasp")
            Fabrica.LogIt.Loguear("Antes", LogPropiedades, LogCategorias)
            ...
            LogPropiedades.Clear()
            LogPropiedades.Agregar("Hora", Now.ToString("dd/MM/yyyy HH:mm:sss"))
            LogPropiedades.Agregar("Mensaje", "Despues de la Hasp")
            Fabrica.LogIt.Loguear("Antes", LogPropiedades, LogCategorias)
                ...
                If OHelperEstacion.RecuperarEstadoTurno(cara.ToString()) = "Abierto" Then
                    ...
                    If Not EsLecturaChipObligatoria Then 'todo: cambiar pr un enumerado
                        If CreditosEstacion.ContainsKey(cara.ToString) Then
                            ...
                            LogPropiedades.Clear()
                            LogPropiedades.Agregar("Hora", Now.ToString("dd/MM/yyyy HH:mm:sss"))
                            LogPropiedades.Agregar("Mensaje", "Entro a la valicdacion de creditos")
    ...
    Sub ListaProtocolos_VentaAutorizada(Cara As Byte, Precio As String, ValorProgramado As String, TipoProgramacion As Byte, Placa As String, MangueraProgramada As Int32, EsVentaGerenciada As Boolean)
        Try
            ...
            For Each Protocolo In ListaSurtidores
                ...
                Protocolo.Evento_VentaAutorizada(Cara, Precio, ValorProgramado, TipoProgramacion, Placa, MangueraProgramada, EsVentaGerenciada)
        ...
        Try
            ...
            Else
                ...
                Try
                    ...
                    Recibo = OHelper.InsertarVenta(cara, CDec(lecturaFinal), CDec(precio), CDec(cantidad), CDec(valor), producto, Guid.NewGuid.ToString, manguera, Kilometraje, TipoDocumento.Identificacion, TipoDocumento.IdentificacionAux, TipoDocumento.TipoIdentificacion, TipoDocumento.FormaPago, Nivel, EsPagoConBono)
                    Reintentar = False

                    If DatosMenoCash.ContainsKey(cara.ToString) Then
                        Dim Gst1 As New GSTCriptografia1.Criptografia1
                        Dim oReaderM As IDataReader
                        Dim servMeno As New ServicioMeno_Cash.IntegracionServiceCore
                        Dim ConfirmacionDTO As New POSstation.ServerServices.ServicioMeno_Cash.confirmacionDTO
                        Dim RespuestaCon As New POSstation.ServerServices.ServicioMeno_Cash.respuestaConfDTO
                        Dim LlavePrivada = OHelper.RecuperarParametro("LlavePrivada")
                        ConfirmacionDTO.direccionIP = OHelper.RecuperarParametro("CodigoSeguridadGasoNet")
                        servMeno.Url = OHelper.RecuperarParametro("UrlMenoCash")
                        oReaderM = OHelper.RecuperarNumeroGasoNet(CInt(cara.ToString()))
                        If oReaderM.Read() Then
                            ConfirmacionDTO.numero = oReaderM("NumeroPreautorizacion")
                            ConfirmacionDTO.fecha = oReaderM("Fecha")
                            ConfirmacionDTO.valor = valor
                            ConfirmacionDTO.kilometros = 0
                            ConfirmacionDTO.precioUnidad = precio
                            RespuestaCon = servMeno.Confirmar(ConfirmacionDTO)
                            Dim Numero = Gst1.DecryptString(RespuestaCon.numero, 1024, LlavePrivada)
                            Dim Nombre = Gst1.DecryptString(RespuestaCon.nombre, 1024, LlavePrivada)
                            Dim PlacaD = Gst1.DecryptString(RespuestaCon.placa, 1024, LlavePrivada)
                            OHelper.InsertarActualizarVentaGasoNet(Nothing, Nothing, RespuestaCon.codigo, Nothing, RespuestaCon.mensaje, Nothing, RespuestaCon.fecha, Nothing, Numero, Nothing, Nothing, Recibo, 2, PlacaD, Nombre, Nothing)
                        End If
                    End If
            ...
            If OCreditoConsumo.Read Then
                ...
                If Not RespuestaCreditoConsumo Is Nothing Then
                    If RespuestaCreditoConsumo.EsAutorizado = True Then
                        ...
                        oEventos_VentaAutorizada(Cara, OHelper.RecuperarPrecioPorCliente(Placa).ToString("N"), CDec(OPreset.Valor).ToString("N2"), OPreset.TipoPreset, Placa, -1, True)
                        EsAutorizado = True
                    Else
                        LogCategorias.Clear()
...
#Region "Nuevos eventos para manejo de ventas de canastilla"
    ...
    Private Sub OEventos_VentaCreditoCanastilla(identificacion As MedioCredito, colProductos As ColCanastilla, Cedula As String, Clave As String, puerto As String) 'Handles OEventos.VentaCreditoCanastilla
        Try
                ...
                Try
                                        ...
                                        If Precio > 0 Then
                                            ...
                                            If CantidadAutorizada <> 0 Then
                                                If CantidadAutorizada > CInt(Item.Cantidad) Then
                                                    ...
                                                    oFactura.AgregarProductoTerpel(CInt(Item.Codigo.ToString()), CInt(Item.Cantidad), "-1")
                                                Else
                                                    ConsumoDisponible -= CantidadAutorizada * Precio
                                                    oFactura.AgregarProductoTerpel(CInt(Item.Codigo.ToString()), CantidadAutorizada, "-1")
                                                End If
                    ...
                    Else
                        For i As Int16 = 1 To colProductos.Count
                            ...
                            oFactura.AgregarProductoTerpel(CInt(ItemCanastilla.Codigo.ToString()), CInt(ItemCanastilla.Cantidad), "-1")
                        Next
        ...
        Try
            If Not OHelper.ExisteVentaParcial(CShort(Cara.ToString)) Then
                ...
                If Not VentaPrepago.ContainsKey(Cara) Then
                    saldo = OHelper.RecuperarSaldoCupo(tarjeta)
                    If saldo >= Valor Then
