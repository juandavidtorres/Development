...
namespace SATELITELite
{
    ...
    /// </summary>
    public sealed partial class MenuTurnoCerrarTanques : Page
    {

        List<Models.TanqueModelo> listaTanquePrueba = new List<Models.TanqueModelo>();
        ...
        private void ListarTanques()
        {
            try
            {
                for (int i = 1; i < 10; i++)
                {
                    listaTanquePrueba.Add(new Models.TanqueModelo("Cod " + i.ToString(), "Tanque " + i.ToString(), "Altura " + i.ToString()));
                }

                listaTanques.ItemsSource = listaTanquePrueba;
        ...
        private void AgregarTanque()
        {
            try
            {
                ...
                string cod = "10";
                string nombre = this.cmbTanques.SelectedItem.ToString();
                string altura = this.txtAltura.Text;                
                listaTanquePrueba.Add(new Models.TanqueModelo("Cod " + cod, nombre, "Altura " + altura));
                listaTanques.ItemsSource = null;
                listaTanques.ItemsSource = listaTanquePrueba;
