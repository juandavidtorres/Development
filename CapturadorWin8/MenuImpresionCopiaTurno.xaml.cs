...
namespace SATELITELite
{
    ...
    /// </summary>
    public sealed partial class MenuImpresionCopiaTurno : Page
    {
        ...
        public MenuImpresionCopiaTurno()
        {
        ...
        private void btn_Click(object sender, RoutedEventArgs e)
        {
            ...
            switch (oBoton.Name)
            {
                case "btnTurnoSurtidor":
                    try
                    {
                        if (!this.Frame.Navigate(typeof(MenuImpresionCopiaTurnoSurtidor)))
                        {
                            throw new Exception("Failed to create MenuImpresionCopiaTurnoSurtidor page");
                        }
                    }
                    catch (Exception ex)
                    {
                    ...
                    break;
                case "btnTurnoEmpleado":
                    try
                    {
                        Funciones.MsgBox("Se solicitaran credenciales del despachador que desea imprimir la copia de turno");
                        if (!this.Frame.Navigate(typeof(MenuImpresionCopiaTurnoEmpleado)))
                        {
                            throw new Exception("Failed to create MenuImpresionCopiaTurnoEmpleado page");
                        }
                    }
                    catch (Exception ex)
                    {
